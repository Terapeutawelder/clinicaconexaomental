...
if (!isset($_GET['servi�o_id']) || !is_numeric($_GET['servi�o_id'])) {
    ...
    try {
        // 1. Verifica se o cliente tem acesso a este servi�o/curso E busca a data de concessão
        $stmt_acesso = $pdo->prepare("
            SELECT data_concessao FROM alunos_acessos 
            WHERE aluno_email = ? AND servi�o_id = ?
        ");
        $stmt_acesso->execute([$cliente_email, $servi�o_id]);
        ...
        if (!$acesso_info) {
            ...
            // 2. Busca os detalhes do curso e o servi�o associado
            $stmt_curso = $pdo->prepare("
                SELECT c.*, p.nome as servi�o_nome, p.descricao as servi�o_descricao, p.foto as servi�o_foto 
                FROM cursos c
...
<head>
    ...
    <style>
        ...
        .module-card.active { border-color: #10b981; box-shadow: 0 0 15px rgba(249, 115, 22, 0.5); }
        ...
        .swiper-button-next, .swiper-button-prev {
            ...
            width: 44px;
            height: 44px;
            background-color: rgba(31, 41, 55, 0.8); /* bg-gray-800 com opacidade */
            border-radius: 50%;
        ...
        .swiper-slide {
            height: auto; /* Garante que os slides tenham altura automática */
        }
        
        /* ===== INÍCIO: PLAYER YOUTUBE CUSTOMIZADO (CSS DO YMin) ===== */
        .ymin{
         --aspect:16/9; --crop:2000px; --accent:#10b981; --bar-color:var(--accent); --track-color:#202532; /* <-- COR LARANJA PRINCIPAL AJUSTADA AQUI */
...
<body class="bg-gray-900 text-gray-200 antialiased">
    ...
    <header class="bg-gray-900 text-white p-6 shadow-md sticky top-0 z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <div class="flex items-center space-x-4">
                <a href="member_area_dashboard.php" class="text-white/90 hover:text-white transition-colors">
            ...
            <div class="flex items-center space-x-4">
                ...
                <a href="member_logout.php" class="flex items-center space-x-2 text-white/90 hover:text-white transition-colors">
        ...
        <div class="flex h-[calc(100vh-64px)] items-center justify-center p-8">
            <div class="bg-red-900 border border-red-700 text-red-200 px-6 py-4 rounded-lg text-center max-w-lg">
                ...
                <p><?php echo $mensagem_erro; ?></p>
                 <a href="member_area_dashboard.php" class="mt-4 inline-block bg-red-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-700 transition">Voltar aos Meus Cursos</a>
            </div>
        </div>
    <?php elseif (!$curso): ?>
        <div class="flex h-[calc(100vh-64px)] items-center justify-center p-8">
    ...
    <div id="course-container" class="min-h-screen">
        ...
        <header class="relative h-64 md:h-80 bg-gray-800 bg-cover bg-center" style="background-image: url('<?php echo htmlspecialchars($curso['banner_url'] ?? ($upload_dir . $curso['servi�o_foto'] ?? '')); ?>')">
            ...
            <div class="relative h-full flex flex-col justify-end p-6 md:p-10 max-w-7xl mx-auto">
                ...
                <p class="mt-2 text-lg text-gray-300 max-w-2xl drop-shadow-md"><?php echo htmlspecialchars($curso['descricao'] ?? $curso['servi�o_descricao']); ?></p>
            </div>
        </header>
        ...
        <main class="max-w-7xl mx-auto p-4 md:p-8 w-full">
            ...
            <?php else: ?>
                ...
                <div id="player-wrapper" class="hidden">
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-semibold text-emerald-400">SEU PROGRESSO</span>
                            <span class="text-sm font-bold text-white"><?php echo $progresso_percentual; ?>% Completo</span>
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            <div class="bg-emerald-500 h-2.5 rounded-full" style="width: <?php echo $progresso_percentual; ?>%"></div>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <!-- Swiper Wrapper para Padding -->
                    <div class="px-4 relative">
                        <!-- Swiper Container para Módulos -->
                        <div class="swiper mySwiper modules-swiper pb-10">
                            <div class="swiper-wrapper">
                                <?php foreach ($modulos_com_aulas as $index => $item): ?>
                                                                    <?php
                                                                    $module = $item['modulo'];
                                                                    $is_module_locked = $module['is_locked'];
                                                                    // Removed 'h-full' to prevent stretching issues on the last item
                                                                    $module_button_classes = "module-card w-full block group relative rounded-lg overflow-hidden border-2 border-gray-700 hover:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-500/50 transition-all duration-300 text-left";
                                        ...
                                        <button class="<?php echo $module_button_classes; ?>" 
                                            ...
                                            <div class="aspect-[3/4] w-full relative">
                                                <?php if (!empty($module['imagem_capa_url']) && file_exists($module['imagem_capa_url'])): ?>
                                                    <img src="<?php echo htmlspecialchars($module['imagem_capa_url']); ?>" alt="Capa do <?php echo htmlspecialchars($module['titulo']); ?>" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110">
                                                <?php else: ?>
                                                ...
                                                <div class="absolute bottom-0 left-0 p-4 w-full">
                                                    <h4 class="font-bold text-lg text-white line-clamp-2"><?php echo htmlspecialchars($module['titulo']); ?></h4>
                                                    ...
                                                    <?php else: ?>
                                                        <span class="text-xs text-emerald-300"><?php echo count($item['aulas']); ?> aulas</span>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </button>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize Swiper for Modules
            var swiper = new Swiper(".modules-swiper", {
                slidesPerView: 2, // Default to 2 for smallest screens
                spaceBetween: 20,
                navigation: {
                ...
                breakpoints: {
                    ...
                    1024: {
                        slidesPerView: 3, // Reduzido de 4 para 3 para aumentar o tamanho
                        spaceBetween: 30,
                    },
                    // when window width is >= 1280px (xl)
                    1280: {
                        slidesPerView: 4, // Reduzido de 5 para 4 para aumentar o tamanho
                        spaceBetween: 30,
                    },
                },
            });
            ...
            const currentProductId = "<?php echo htmlspecialchars($servi�o_id); ?>";
            ...
            function loadLesson(lesson) {
                ...
                let descriptionHtml = (lesson.descricao || 'Esta aula não possui descrição.')
                    ...
                    .replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" class="text-emerald-500 hover:underline">$1</a>') // Link detection
                    .replace(/\n/g, '<br>');
                ...
                if ((lesson.tipo_conteudo === 'files' || lesson.tipo_conteudo === 'mixed') && lesson.files && lesson.files.length > 0) {
                    ...
                    lesson.files.forEach(file => {
                        ...
                        descriptionHtml += `
                            <a href="${filePath}" target="_blank" class="bg-emerald-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-emerald-700 transition duration-300 text-base flex items-center justify-center space-x-2">
            ...
            markAsCompleteBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`member_api.php?action=${action}`, {
                        method: 'POST',
            ...
            function updateOverallProgress() {
                ...
                document.querySelector('#player-wrapper .bg-emerald-500').style.width = `${newProgressPercent}%`;
            }
