...
// Busca todos os servi�os do infoservi�or para o dropdown de associação de integração UTMfy
try {
    ...
    $mensagem = "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>Erro ao buscar servi�os para UTMfy: " . htmlspecialchars($e->getMessage()) . "</div>";
...
<div class="container mx-auto">
    <div class="flex items-center mb-6">
        <a href="index.php?pagina=integracoes" class="text-blue-600 hover:text-blue-800 mr-4">
            <i data-lucide="arrow-left-circle" class="w-8 h-8"></i>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-800">Integrar com UTMfy</h1>
            <p class="text-gray-600">Conecte a Mentalpag à UTMfy para enviar notificações de vendas e rastrear links.</p>
        </div>
    </div>
    ...
    <div class="bg-white p-8 rounded-lg shadow-md mb-8">
        <h2 id="form-title" class="text-2xl font-semibold text-gray-800 mb-6">Adicionar Nova Integração UTMfy</h2>
        <form id="utmfy-form" class="space-y-6">
            <input type="hidden" name="utmfy_integration_id" id="utmfy_integration_id">
            
            <div>
                <label for="integration_name" class="block text-gray-700 text-sm font-semibold mb-2">Nome da Integração</label>
                ...
                <p class="text-xs text-gray-500 mt-1">Um nome amigável para identificar esta integração.</p>
            </div>

            <div>
                <label for="api_token" class="block text-gray-700 text-sm font-semibold mb-2">API Token da UTMfy</label>
                <input type="text" id="api_token" name="api_token" required
                       class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="Cole seu API Token da UTMfy aqui">
                <p class="text-xs text-gray-500 mt-1">Este token é fornecido pela plataforma UTMfy. Para obtê-lo, acesse o painel da UTMfy em Integrações > Webhooks > Credenciais de API.</p>
            </div>

            <div>
                <label for="product_id" class="block text-gray-700 text-sm font-semibold mb-2">Servi�o Associado (Opcional)</label>
                <select id="product_id" name="product_id"
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">Todos os servi�os (Integração Global)</option>
                ...
                <p class="text-xs text-gray-500 mt-1">Selecione um servi�o para que a UTMfy seja notificada apenas para vendas dele. Deixe vazio para notificar para todos os servi�os.</p>
            </div>

            <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-3">Eventos que Disparam a Notificação para UTMfy</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_initiate_checkout" class="form-checkbox h-5 w-5 text-indigo-600 rounded">
                        <span class="ml-2 text-gray-700">Carrinho Abandonado (InitiateCheckout)</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-4 mt-6">
                ...
                <button type="submit" id="save-utmfy-btn" class="bg-blue-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-blue-700 transition duration-300 flex items-center space-x-2">
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Integrações UTMfy Cadastradas</h2>
        ...
        <div id="empty-state" class="text-center py-12 text-gray-500" style="display: none;">
            <i data-lucide="link" class="mx-auto w-16 h-16 text-gray-300 mb-2"></i>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateFormForEdit(integration) {
        ...
        productIdSelect.value = integration.product_id || ''; // Se for null, seleciona a opção "Todos os servi�os"
    ...
    async function fetchUtmfyIntegrations() {
        showLoading();
        try {
            const response = await fetch('api.php?action=get_utmfy_integrations');
            ...
            if (result.success) {
                ...
                if (cachedIntegrations.length === 0) {
                    ...
                    cachedIntegrations.forEach(integration => {
                        ...
                        integrationItem.className = 'bg-gray-50 p-4 rounded-lg border border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center';
                        
                        const productName = integration.product_name ? htmlspecialchars(integration.product_name) : '<span class="text-gray-500">(Todos os servi�os)</span>';
                        const activeEvents = getEventStatusText(integration);

                        integrationItem.innerHTML = `
                            <div class="flex-1 mb-2 sm:mb-0">
                                <p class="font-semibold text-gray-800 text-lg">${htmlspecialchars(integration.name)}</p>
                                <p class="text-sm text-gray-600 mt-1">API Token: <span class="font-mono text-gray-700">${htmlspecialchars(integration.api_token).substring(0, 10)}...</span></p>
                                <p class="text-sm text-gray-600">Servi�o: ${productName}</p>
                                <p class="text-sm text-gray-600">Eventos: ${activeEvents}</p>
                            </div>
                            <div class="flex space-x-2 mt-2 sm:mt-0 flex-shrink-0">
                                <button type="button" class="edit-utmfy-btn bg-yellow-100 text-yellow-700 font-semibold py-2 px-4 rounded-lg hover:bg-yellow-200 transition text-sm flex items-center space-x-1" data-utmfy-id="${integration.id}">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                    <span>Editar</span>
                                </button>
                                <button type="button" class="delete-utmfy-btn bg-red-100 text-red-700 font-semibold py-2 px-4 rounded-lg hover:bg-red-200 transition text-sm flex items-center space-x-1" data-utmfy-id="${integration.id}">
    ...
    async function deleteUtmfyIntegration(integrationId) {
        ...
        try {
            const response = await fetch('api.php?action=delete_utmfy_integration', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
