...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Sua Jornada Starfy</h1>

    <!-- O container principal (Estágio Atual) permanece o mesmo, pois já está ótimo! -->
    <div id="jornada-Starfy-container" class="bg-gradient-to-br from-purple-800 via-indigo-900 to-black p-6 rounded-lg shadow-md mb-8 text-white relative overflow-hidden transition-colors duration-1000 ease-in-out">
        ...
        <div class="relative z-10">
            <h2 class="text-3xl font-extrabold text-white mb-4">Seu Caminho Estelar no Starfy</h2>
            ...
            <div class="mt-6">
                <div class="w-full bg-white rounded-full h-3.5 mb-2 relative">
                    <div id="journey-progress-bar-fill" class="bg-emerald-500 h-3.5 rounded-full transition-all duration-700 ease-out" style="width: 0%;"></div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-8">
        ...
        <p class="text-gray-600 mb-6">Veja onde você está e quais são seus próximos passos no universo Starfy.</p>
...
document.addEventListener('DOMContentLoaded', function() {
    // Mapeamento das etapas da jornada Starfy (sem alteração)
    const Starfy_STAGES = [
        { name: 'ESTRELA CADENTE', min: 0, max: 10000, description: 'Toda grande jornada começa com um vislumbre. Você acendeu a sua luz no universo Starfy. É o ponto de partida de uma trajetória que promete ser épica.', color_classes: 'from-purple-800 via-indigo-900 to-black' },
        { name: 'NEBULOSA ASCENDENTE', min: 10000, max: 100000, description: 'Uma formação se inicia, cores vibrantes e um brilho crescente. Você está moldando seu espaço na galáxia Starfy, com potencial para se expandir e impressionar.', color_classes: 'from-indigo-900 via-blue-950 to-gray-900' },
        ...
        { name: 'SUPERNOVA DOURADA', min: 500000, max: 1000000, description: 'Um evento cósmico de proporções grandiosas! Sua energia explode, iluminando vastas extensões. Você é uma força que redefine o seu entorno, uma referência inconfundível.', color_classes: 'from-yellow-400 via-amber-600 to-emerald-700' },
        { name: 'GALÁXIA SUPREMA', min: 1000000, max: 5000000, description: 'Você se tornou um universo em si, com trilhões de estrelas e uma complexidade que inspira. Uma galáxia construída com visão, resiliência e paixão, reconhecida por sua vastidão.', color_classes: 'from-emerald-400 via-red-600 to-rose-700' },
    ...
    async function fetchJornadaStarfyData() {
        try {
            const response = await fetch('api.php?action=get_jornada_Starfy_data');
            if (!response.ok) {
            ...
            document.getElementById('jornada-Starfy-container').classList.add('from-red-600', 'via-red-700', 'to-red-800');
        }
    }
    ...
    function renderStarfyJourney(totalFaturamentoLifetime) {
        ...
        if (!currentStage && totalFaturamentoLifetime >= Starfy_STAGES[Starfy_STAGES.length - 1].min) {
            currentStage = Starfy_STAGES[Starfy_STAGES.length - 1];
        } else if (!currentStage) { 
             currentStage = Starfy_STAGES[0];
        }
        ...
        if (currentStage) {
            ...
            const journeyContainer = document.getElementById('jornada-Starfy-container');
        ...
        // A chamada para renderAllStages foi movida para fetchJornadaStarfyData
    }
    ...
    function renderAllStagesTimeline(currentFaturamento) {
        ...
        Starfy_STAGES.forEach((stage, index) => {
            let status = 'locked';
            let iconName = 'lock';
            ...
            if (currentFaturamento >= stage.min && currentFaturamento < stage.max) {
                ...
                status = 'current';
                iconName = 'star';
                iconClasses = 'text-emerald-500 animate-pulse';
                dotClasses = 'bg-emerald-100 border-emerald-500';
                cardClasses = 'bg-white border-emerald-500 shadow-md shadow-emerald-100';
            ...
            if (index < Starfy_STAGES.length - 1) {
