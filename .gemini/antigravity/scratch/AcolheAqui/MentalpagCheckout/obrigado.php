    ...
    try {
        $stmt = $pdo->prepare("
            SELECT 
                ...
                p.id as serviço_id,
                p.nome as serviço_nome,
                p.checkout_config
            FROM vendas v
            JOIN serviços p ON v.serviço_id = p.id
        ...
        if ($sale_details) {
            ...
            $stmt_get_Starfy_tracking = $pdo->prepare("SELECT stp.tracking_id FROM Starfy_tracking_products stp JOIN serviços p ON stp.serviço_id = p.id WHERE p.id = ?");
            $stmt_get_Starfy_tracking->execute([$sale_details['serviço_id']]);
        ...
        // Em um ambiente de produÃ§Ã£o, Ã© melhor logar este erro do que exibi-lo.
    ...
    <?php if ($sale_details): ?>
        ...
        <script>
          ...
          gtag('event', 'purchase', {
            ...
            "currency": "BRL",
            "items": [{
              "item_id": "<?php echo htmlspecialchars($sale_details['serviço_id']); ?>",
              "item_name": "<?php echo htmlspecialchars($sale_details['serviço_nome']); ?>",
        ...
        <script>
            (function() {
                ...
                const PRODUCT_ID = <?php echo (int)$sale_details['serviço_id']; ?>;
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-4">
    <div class="w-full max-w-lg p-6 sm:p-8 bg-white rounded-2xl shadow-lg text-center">
        ...
        <p class="text-gray-600 mt-4 text-lg">
            Obrigado pela sua compra! Em breve vocÃª receberÃ¡ um e-mail com todos os detalhes e o acesso ao seu serviço.
        </p>
        
        <?php if($sale_details && $sale_details['serviço_nome']): ?>
            <div class="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                <p class="text-sm text-gray-500">Serviço adquirido:</p>
                <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($sale_details['serviço_nome']); ?></p>
            </div>
        <?php endif; ?>
        ...
        <?php if($payment_id): ?>
            <p class="text-xs text-gray-400 mt-10">ID da transaÃ§Ã£o: <?php echo htmlspecialchars($payment_id); ?></p>
        <?php endif; ?>
    </div>
    <script>
        lucide.createIcons();
