...
if (!isset($_GET['servi�o_id']) || !is_numeric($_GET['servi�o_id'])) {
    ...
    try {
        // Busca o curso correspondente ao servi�o
...
<head>
    ...
    <style>
        ...
        .prose { --tw-prose-body: #d1d5db; --tw-prose-headings: #f9fafb; --tw-prose-links: #34d399; } /* Estilos para o texto da descrição */
        ...
        <div class="flex h-screen items-center justify-center p-8">
            <div class="bg-red-900 border border-red-700 text-red-200 px-6 py-4 rounded-lg text-center max-w-lg">
                ...
                <p><?php echo $mensagem_erro; ?></p>
                 <a href="index.php?pagina=area_membros" class="mt-4 inline-block bg-red-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-700 transition">Voltar</a>
            </div>
        </div>
    <?php elseif (!$curso): ?>
        <div class="flex h-screen items-center justify-center p-8">
    ...
    <div id="course-container" class="min-h-screen">
        ...
        <header class="relative h-64 md:h-80 bg-gray-800 bg-cover bg-center" style="background-image: url('<?php echo htmlspecialchars($curso['banner_url'] ?? ''); ?>')">
            ...
            <div class="relative h-full flex flex-col justify-end p-6 md:p-10 max-w-7xl mx-auto">
                 <a href="index.php?pagina=gerenciar_curso&servi�o_id=<?php echo $servi�o_id; ?>" class="absolute top-4 right-4 bg-black/50 text-white font-semibold py-2 px-4 rounded-lg hover:bg-black/80 transition duration-300 flex items-center space-x-2 text-sm z-10">
