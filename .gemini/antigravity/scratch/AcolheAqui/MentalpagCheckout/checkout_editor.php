...
// 1. Validar e buscar o servi�o
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header("Location: index.php?pagina=servi�os");
    exit;
}
...
try {
    // 2. Buscar o servi�o e verificar se ele pertence ao usuário logado
    $stmt = $pdo->prepare("SELECT * FROM servi�os WHERE id = ? AND usuario_id = ?");
    $stmt->execute([$id_servi�o, $usuario_id_logado]);
    $servi�o = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    // Identifica o gateway do servi�o atual
    $current_gateway = $servi�o['gateway'] ?? 'mercadopago';

    // --- ALTERAÇÃO AQUI: Filtra servi�os de Order Bump pelo mesmo Gateway ---
    $stmt_todos_servi�os = $pdo->prepare("SELECT id, nome FROM servi�os WHERE id != ? AND usuario_id = ? AND gateway = ?");
    $stmt_todos_servi�os->execute([$id_servi�o, $usuario_id_logado, $current_gateway]);
    $lista_servi�os_orderbump = $stmt_todos_servi�os->fetchAll(PDO::FETCH_ASSOC);

    // Busca os order bumps existentes para este servi�o
    $stmt_ob = $pdo->prepare("SELECT * FROM order_bumps WHERE main_product_id = ? ORDER BY ordem ASC");
    $stmt_ob->execute([$id_servi�o]);
...
if (isset($_POST['salvar_checkout'])) {
    ...
    try {
        ...
        $stmt_get_config = $pdo->prepare("SELECT checkout_config FROM servi�os WHERE id = ? AND usuario_id = ?");
        $stmt_get_config->execute([$id_servi�o, $usuario_id_logado]);
        ...
        $newly_uploaded_banners = handle_multiple_uploads('add_banner_files', 'banner', $id_servi�o);
        ...
        $newly_uploaded_side_banners = handle_multiple_uploads('add_side_banner_files', 'sidebanner', $id_servi�o);
        ...
        $stmt_update_servi�o = $pdo->prepare("UPDATE servi�os SET preco_anterior = ? WHERE id = ? AND usuario_id = ?");
        $stmt_update_servi�o->execute([$preco_anterior, $id_servi�o, $usuario_id_logado]);
        ...
        $stmt = $pdo->prepare("UPDATE servi�os SET checkout_config = ? WHERE id = ? AND usuario_id = ?");
        $stmt->execute([$config_json, $id_servi�o, $usuario_id_logado]);
        
        // --- LÓGICA DO ORDER BUMP (MÚLTIPLOS) ---
        $stmt_delete_ob = $pdo->prepare("DELETE FROM order_bumps WHERE main_product_id = ?");
        $stmt_delete_ob->execute([$id_servi�o]);
        ...
        if (isset($_POST['orderbump_product_id']) && is_array($_POST['orderbump_product_id'])) {
            ...
            foreach ($_POST['orderbump_product_id'] as $index => $ob_product_id) {
                ...
                // Valida se o servi�o de order bump também pertence ao usuário E se tem o MESMO gateway
                $stmt_check_owner = $pdo->prepare("SELECT id FROM servi�os WHERE id = ? AND usuario_id = ? AND gateway = ?");
                ...
                if($stmt_check_owner->rowCount() > 0) {
                    ...
                    $stmt_insert_ob->execute([$id_servi�o, $ob_product_id, $headline, $description, $ordem, $is_active]);
                    $ordem++;
                }
            }
        }
...
// Recarrega a lista de servi�os para o select (caso tenha adicionado um servi�o que mudou de gateway)
$stmt_todos_servi�os = $pdo->prepare("SELECT id, nome FROM servi�os WHERE id != ? AND usuario_id = ? AND gateway = ?");
$stmt_todos_servi�os->execute([$id_servi�o, $usuario_id_logado, $current_gateway]);
$lista_servi�os_orderbump = $stmt_todos_servi�os->fetchAll(PDO::FETCH_ASSOC);
...
<div class="flex h-screen bg-gray-100 font-sans">
    <div class="w-1/3 h-full bg-white shadow-lg overflow-y-auto">
        <form action="index.php?pagina=checkout_editor&id=<?php echo $id_servi�o; ?>" method="post" enctype="multipart/form-data" class="p-6">
            <div class="flex items-center mb-6 border-b pb-4">
                <a href="index.php?pagina=servi�os" class="text-indigo-600 hover:text-indigo-800 mr-4">
                     <i data-lucide="arrow-left-circle" class="w-7 h-7"></i>
                </a>
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Editor de Checkout</h1>
                    <p class="text-sm text-gray-600 flex items-center gap-2">
                        Servi�o: <?php echo htmlspecialchars($servi�o['nome']); ?>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="space-y-4 p-4 border rounded-lg bg-gray-50">
                        <div>
                            <label for="summary_product_name" class="block text-gray-700 text-sm font-semibold mb-2">Nome do Servi�o no Checkout</label>
                            <input type="text" id="summary_product_name" name="summary_product_name" class="form-input-sm" value="<?php echo htmlspecialchars($checkout_config['summary']['product_name'] ?? $servi�o['nome']); ?>">
                             <p class="text-xs text-gray-500 mt-1">Por padrão, usa o nome original do servi�o.</p>
                        </div>
                        <div>
                            <label for="preco_anterior" class="block text-gray-700 text-sm font-semibold mb-2">Preço Original (De)</label>
                            <input type="text" id="preco_anterior" name="preco_anterior" class="form-input-sm" placeholder="Ex: 99,90" value="<?php echo !empty($servi�o['preco_anterior']) ? htmlspecialchars(number_format($servi�o['preco_anterior'], 2, ',', '.')) : ''; ?>">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Exibido como um selo de destaque no servi�o.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-4 border rounded-lg bg-gray-50 space-y-4">
                        ...
                        <?php if($current_gateway == 'pushinpay'): ?>
                            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
                                <p class="text-sm text-blue-700">Listando apenas servi�os configurados com <strong>PushinPay</strong>.</p>
                            </div>
                        <?php endif; ?>
                        ...
                        <div id="order-bumps-container">
                            <?php foreach ($order_bumps as $index => $bump): ?>
                                <div class="order-bump-item p-4 border rounded-lg bg-white mb-3" data-index="<?php echo $index; ?>">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            <label class="block text-gray-700 text-sm font-semibold mb-2">Servi�o da Oferta</label>
                                            <select name="orderbump_product_id[]" class="form-input-sm">
                                                <option value="">-- Selecione um servi�o --</option>
                                                <?php foreach ($lista_servi�os_orderbump as $prod_ob): ?>
                            ...
                            <div class="bg-emerald-50 border-l-4 border-emerald-500 p-4 mb-4">
                                <div class="flex">
                                    <div class="flex-shrink-0">
                                        <i data-lucide="alert-circle" class="text-emerald-500 h-5 w-5"></i>
                                    </div>
                                    <div class="ml-3">
                                        <p class="text-sm text-emerald-700">
                                            Este servi�o usa o gateway <strong>PushinPay</strong>. Apenas o método <strong>Pix</strong> está disponível e habilitado automaticamente.
                                        </p>
                                    </div>
                                </div>
                    ...
                    <div class="p-4 border rounded-lg bg-gray-50">
                         <div class="flex items-start space-x-4">
                            ...
                            <div class="text-sm">
                                ...
                                <p class="text-gray-500">Mostra pop-ups de pessoas comprando o servi�o.</p>
                            </div>
                        </div>
                        <div class="mt-4 space-y-3">
                             <div>
                                 <label for="sales_notification_product" class="block text-gray-700 text-sm font-semibold mb-2">Nome do Servi�o na Notificação</label>
                                 <input type="text" name="sales_notification_product" id="sales_notification_product" value="<?php echo htmlspecialchars($checkout_config['salesNotification']['product'] ?? $servi�o['nome']); ?>" class="form-input-sm">
                             </div>
                             <div>
    ...
    <div class="w-2/3 h-full p-8 bg-gray-200 flex items-center justify-center">
        <div class="w-full max-w-2xl h-[90%] bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div class="h-8 bg-gray-800 flex items-center px-4">
             ...
             <iframe id="checkout-preview" src="checkout.php?p=<?php echo $servi�o['checkout_hash']; ?>&preview=true&rand=<?php echo time(); ?>" class="w-full h-full border-0"></iframe>
        </div>
    </div>
</div>
<style>
