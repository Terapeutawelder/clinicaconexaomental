...
try {
    ...
    $stmt_tracking_product = $pdo->prepare("SELECT id FROM mentalpag_tracking_products WHERE tracking_id = :tracking_id");
    ...
    // 4. Inserir o evento na tabela mentalpag_tracking_events
    // Usamos um SELECT COUNT para verificar se o evento já existe para esta sessão e servi�o
    // para evitar duplicações em refreshes da página.
    $check_existing_stmt = $pdo->prepare("SELECT COUNT(*) FROM mentalpag_tracking_events WHERE tracking_product_id = :tracking_product_id AND session_id = :session_id AND event_type = :event_type");
    ...
    $stmt_insert = $pdo->prepare("INSERT INTO mentalpag_tracking_events (tracking_product_id, session_id, event_type, event_data) VALUES (:tracking_product_id, :session_id, :event_type, :event_data)");
