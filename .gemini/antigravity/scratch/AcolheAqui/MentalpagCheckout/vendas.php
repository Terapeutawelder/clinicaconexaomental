...
<div class="container mx-auto relative">
    ...
    <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
        <div data-status="all" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-emerald-500">
            <h3 class="text-gray-500 text-sm font-medium">Total de Vendas</h3>
            <p id="metric-all" class="text-2xl font-bold text-gray-800">0</p>
        </div>
        <div data-status="approved" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-transparent hover:border-gray-300">
            <h3 class="text-gray-500 text-sm font-medium">Aprovadas</h3>
            <p id="metric-approved" class="text-2xl font-bold text-green-600">0</p>
        </div>
        <div data-status="abandoned_all" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-transparent hover:border-gray-300">
            <h3 class="text-gray-500 text-sm font-medium">Abandonadas</h3>
            <p id="metric-abandoned_all" class="text-2xl font-bold text-red-600">0</p>
        </div>
        <div data-status="info_filled" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-transparent hover:border-gray-300">
            <h3 class="text-gray-500 text-sm font-medium">Checkout Aband.</h3>
            <p id="metric-info_filled" class="text-2xl font-bold text-red-400">0</p>
        </div>
        <div data-status="refunded" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-transparent hover:border-gray-300">
            <h3 class="text-gray-500 text-sm font-medium">Reembolsadas</h3>
            <p id="metric-refunded" class="text-2xl font-bold text-purple-600">0</p>
        </div>
        <div data-status="charged_back" class="metric-card p-4 bg-white rounded-lg shadow-md cursor-pointer border-2 border-transparent hover:border-gray-300">
            <h3 class="text-gray-500 text-sm font-medium">Chargeback</h3>
            <p id="metric-charged_back" class="text-2xl font-bold text-pink-600">0</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <div class="mb-4">
            <div class="relative">
                ...
                <input type="text" id="search-input" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm" placeholder="Pesquisar por nome, e-mail ou ID...">
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Servi�o</th>
                ...
                <tbody id="vendas-tbody" class="bg-white divide-y divide-gray-200"></tbody>
            </table>
        </div>

        <div id="loading-state" class="text-center py-12 text-gray-500" style="display: none;">
            <svg class="animate-spin h-8 w-8 text-emerald-500 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.96l2-2.669z"></path></svg>
        ...
        <div id="pagination-controls" class="hidden mt-4 flex items-center justify-between">
            <button id="prev-page" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Anterior</button>
            <span id="page-info" class="text-sm text-gray-700">Página 1 de 1</span>
            <button id="next-page" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Próximo</button>
        </div>
    </div>
</div>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Reenviar Acesso</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500 mb-4">O cliente cadastrou o e-mail errado? Corrija abaixo para reenviar o acesso.</p>
                            <label for="modal-email-input" class="block text-sm font-medium text-gray-700">E-mail de Destino</label>
                            <input type="email" id="modal-email-input" class="mt-1 focus:ring-emerald-500 focus:border-emerald-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md border p-2" placeholder="email@cliente.com">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" id="confirm-resend-btn" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-emerald-600 text-base font-medium text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 sm:ml-3 sm:w-auto sm:text-sm">
                    Enviar Agora
                </button>
                <button type="button" id="cancel-resend-btn" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                    Cancelar
                </button>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fetchVendas = async () => {
        ...
        try {
            const url = `api.php?action=get_vendas&status=${state.status}&search=${encodeURIComponent(state.search)}&page=${state.page}`;
            ...
            if (data.vendas.length === 0) {
                ...
                data.vendas.forEach(venda => {
                    ...
                    if (venda.status_pagamento === 'approved' || venda.status_pagamento === 'paid' || venda.status_pagamento === 'completed') {
                        actionsHtml += `
                            <button class="resend-btn text-emerald-600 hover:text-emerald-800 p-1" title="Reenviar Acesso" onclick="openResendModal(${venda.id}, '${venda.comprador_email}')">
                    ...
                    tr.innerHTML = `
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">${venda.comprador_nome || 'Não informado'}</div>
                            <div class="text-sm text-gray-500">${venda.comprador_email || ''}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-700">${venda.servi�o_nome || 'Servi�o não encontrado'}</div></td>
                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-semibold text-gray-900">${valorFormatado}</div></td>
                        <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-700 flex items-center"><img src="${iconUrl}" class="w-5 h-5 mr-2">${metodo}</div></td>
    ...
    window.approveSale = async (vendaId) => {
        ...
        try {
            const res = await fetch('vendas_actions.php', {
                method: 'POST',
    ...
    metricCardsContainer.addEventListener('click', (e) => {
        ...
        if (card) {
            document.querySelectorAll('.metric-card').forEach(c => { c.classList.remove('border-emerald-500'); c.classList.add('border-transparent', 'hover:border-gray-300'); });
            card.classList.add('border-emerald-500'); card.classList.remove('border-transparent', 'hover:border-gray-300');
