...
if (isset($_POST['salvar_perfil'])) {
    ...
    try {
        ...
        $mensagem = "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>Erro ao atualizar perfil: " . $e->getMessage() . "</div>";
...
<div class="container mx-auto">
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
        <form action="index.php?pagina=profile" method="post" enctype="multipart/form-data">
            <div class="mb-8 flex flex-col items-center">
                <h2 class="text-2xl font-semibold mb-4 text-gray-800">Sua Foto de Perfil</h2>
                
                <div class="relative w-40 h-40 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden mb-4 border-4 border-gray-300 shadow-lg">
                    ...
                    <?php else: ?>
                        <i data-lucide="user-circle" class="w-24 h-24 text-gray-400"></i>
                    <?php endif; ?>
                ...
                <p class="text-sm text-gray-600 mb-4">Clique na foto para alterar. Max: 2MB.</p>

                <?php if (!empty($current_foto_perfil)): ?>
                    <div class="flex items-center mb-6">
                        <input type="checkbox" id="remove_foto_perfil" name="remove_foto_perfil" value="1" class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded mr-2">
                        <label for="remove_foto_perfil" class="text-sm text-red-600 font-medium">Remover foto de perfil</label>
                    </div>
                <?php endif; ?>
            </div>

            <div class="mb-6">
                <label for="nome" class="block text-gray-700 text-sm font-semibold mb-2">Seu Nome</label>
                <input type="text" id="nome" name="nome"
                       class="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 transition duration-200"
                       value="<?php echo $current_name; ?>" required placeholder="Seu nome completo">
            </div>

            <div class="mb-6">
                <label for="email" class="block text-gray-700 text-sm font-semibold mb-2">Seu E-mail (Usuário de Login)</label>
                ...
                <p class="text-xs text-gray-500 mt-1">Seu e-mail não pode ser alterado por aqui.</p>
            </div>
            
            <div class="mt-8 pt-6 border-t">
                <button type="submit" name="salvar_perfil" class="bg-emerald-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-emerald-700 transition duration-300">
                    Salvar Alterações
                </button>
            </div>
        </form>
...
<div id="profile-success-alert" class="hidden fixed top-4 left-1/2 -translate-x-1/2 bg-green-500 text-white p-4 rounded-lg shadow-xl z-50"></div>
