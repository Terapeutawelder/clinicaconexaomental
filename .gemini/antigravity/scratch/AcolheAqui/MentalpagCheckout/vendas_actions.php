...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    log_action("Erro: NÃ£o autorizado.");
...
if (file_exists('utmfy_helper.php')) {
    ...
    log_action("ERRO: utmfy_helper.php nÃ£o encontrado.");
}
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    if ($type === 'link') {
        return ['success' => !empty($content), 'product_name' => $product_data['serviço_nome'], 'content_type' => 'link', 'content_value' => $content];
    }
    if ($type === 'email_pdf') {
        return ['success' => file_exists('uploads/'.$content), 'product_name' => $product_data['serviço_nome'], 'content_type' => 'pdf', 'content_value' => 'uploads/'.$content];
    }
    if ($type === 'area_membros') {
        $pdo->prepare("INSERT IGNORE INTO alunos_acessos (aluno_email, serviço_id) VALUES (?, ?)")->execute([$customer_email, $product_data['serviço_id']]);
        return ['success' => true, 'product_name' => $product_data['serviço_nome'], 'content_type' => 'area_membros', 'content_value' => null];
    }
...
function manual_send_email($to_email, $customer_name, $products, $password, $login_url) {
    ...
    try {
        ...
        $mail->setFrom($fromEmail, $conf['smtp_from_name'] ?? 'Mentalpag');
        $mail->CharSet = 'UTF-8';
        $mail->addAddress($to_email, $customer_name); 
        $mail->Subject = $conf['email_template_delivery_subject'] ?? 'Seu acesso chegou!';
        $mail->isHTML(true);

        $template = $conf['email_template_delivery_html'] ?? '<p>OlÃ¡ {CLIENT_NAME}, seus serviços:</p><!-- LOOP_PRODUCTS_START --><p>{PRODUCT_NAME}: <a href="{PRODUCT_LINK}">{PRODUCT_LINK}</a></p><!-- LOOP_PRODUCTS_END -->';
...
// Valida se a venda pertence ao usuÃ¡rio logado
// IMPORTANTE: Buscamos p.nome as serviço_nome para usar no email e no UTMfy
$stmt = $pdo->prepare("SELECT v.*, p.nome as serviço_nome, p.tipo_entrega, p.conteudo_entrega, p.usuario_id FROM vendas v JOIN serviços p ON v.serviço_id = p.id WHERE v.id = ?");
...
// Recupera todas as vendas da mesma sessÃ£o
$stmt_session = $pdo->prepare("SELECT v.*, p.nome as serviço_nome, p.tipo_entrega, p.conteudo_entrega, p.id as pid FROM vendas v JOIN serviços p ON v.serviço_id = p.id WHERE v.checkout_session_uuid = ?");
...
if ($action === 'approve' || $action === 'resend') {
    ...
    if ($action === 'approve') {
        ...
        if (function_exists('trigger_utmfy_integrations')) {
            ...
            $event_data = [
                ...
                // Monta o serviço explicitamente com 'nome'
                'serviços_comprados' => [
                    [
                        'serviço_id' => $venda_data['serviço_id'],
                        'nome' => $venda_data['serviço_nome'], // Garante que o nome seja passado
            ...
            trigger_utmfy_integrations($usuario_id_logado, $event_data, 'approved', $venda_data['serviço_id']);
        }
    ...
    if ($send_result['success']) {
        ...
        echo json_encode(['success' => true, 'message' => 'AÃ§Ã£o realizada com sucesso.']);
    } else {
        echo json_encode(['success' => false, 'message' => 'AÃ§Ã£o processada, mas falha no envio do e-mail: ' . $send_result['error']]);
    }

} else {
    echo json_encode(['success' => false, 'message' => 'AÃ§Ã£o desconhecida.']);
}
?>
