<?php
// =================================================================================
// ARQUIVO AUXILIAR: LÃ³gica de envio para UTMfy
...
if (!function_exists('trigger_utmfy_integrations')) {
    /**
     * Dispara eventos para integraÃ§Ã£o com a UTMfy.
     */
    function trigger_utmfy_integrations($usuario_id, $event_data, $trigger_event, $serviço_id = null, $gateway_data = []) {
        global $pdo;
        ...
        log_utmfy_helper("Evento: '$trigger_event' -> Status UTMfy: '$utmfy_status' | ID TransaÃ§Ã£o: " . ($event_data['transacao_id'] ?? 'N/A'));

        // 3. MAPEAMENTO DE COLUNAS (CORREÃ‡ÃƒO DO ERRO SQL)
        // Define qual coluna do banco verificar (event_approved, event_pending, etc)
        $event_field = 'event_' . $trigger_event; 
        
        // CORREÃ‡ÃƒO: Se for aprovado, usa 'event_approved' (que existe no banco) em vez de 'event_purchase'
        ...
        try {
            ...
            $stmt->execute([':uid' => $usuario_id, ':pid' => $serviço_id]);
            $integrations = $stmt->fetchAll(PDO::FETCH_ASSOC);

            if (empty($integrations)) {
                log_utmfy_helper("AVISO: Nenhuma integraÃ§Ã£o ativa encontrada para User: $usuario_id na coluna '$event_field'.");
                return;
            }

            // 4. PREPARAÃ‡ÃƒO DO PAYLOAD
            ...
            if (!empty($event_data['serviços_comprados']) && is_array($event_data['serviços_comprados'])) {
                foreach ($event_data['serviços_comprados'] as $prod) {
                    $cents = (int)(round((float)$prod['valor'], 2) * 100);
                    
                    // Garante que o nome nÃ£o vÃ¡ vazio
                    $prod_name = !empty($prod['nome']) ? $prod['nome'] : ($prod['serviço_nome'] ?? '');
                    if (empty($prod_name) || trim($prod_name) === '') {
                        $prod_name = "Serviço ID " . ($prod['serviço_id'] ?? 'Unknown');
                    }

                    $products_payload[] = [
                        'id' => (string)($prod['serviço_id'] ?? $serviço_id), 
            ...
            foreach ($integrations as $int) {
                log_utmfy_helper("Enviando para: " . $int['name']);
