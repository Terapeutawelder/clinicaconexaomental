 ...
 * @param array $checkout_config Configurações do checkout do servi�o.
 */
function handle_tracking_events($status, $sale_details, $checkout_config) {
    ...
    if (empty($tracking_config)) {
        log_webhook("TRACKING: Configuração de rastreamento vazia para o servi�o.");
        return;
    }
    ...
    if (!empty($fb_events_enabled[$event_key])) {
        ...
        $checkout_url = $protocol . $domainName . $path . '/checkout.php?p=' . $sale_details['checkout_hash'];
        
        sendFacebookConversionEvent($pixel_id, $api_token, $fb_event_name, $sale_details, $checkout_url);
    } else {
        log_webhook("TRACKING FB: Evento '$fb_event_name' (chave: $event_key) está desativado nas configurações do servi�o.");
    }
}
 ...
 *
 * @param int $usuario_id ID do infoservi�or.
 ...
 * @param int|null $servi�o_id O ID do servi�o associado à venda, se houver.
 */
function trigger_webhooks($usuario_id, $event_data, $trigger_event, $servi�o_id = null) {
    global $pdo;
    log_webhook("WEBHOOKS: Verificando webhooks para o evento '$trigger_event' (usuario_id: $usuario_id, servi�o_id: " . ($servi�o_id ?? 'NULL') . ")");

    $event_field = 'event_' . $trigger_event;

    // Busca webhooks globais (servi�o_id IS NULL) ou específicos para o servi�o
    $stmt_webhooks = $pdo->prepare("
        SELECT url 
        FROM webhooks 
        WHERE usuario_id = :usuario_id 
        AND {$event_field} = 1 
        AND (servi�o_id IS NULL OR servi�o_id = :servi�o_id)
    ");
    $stmt_webhooks->bindParam(':usuario_id', $usuario_id, PDO::PARAM_INT);
    $stmt_webhooks->bindParam(':servi�o_id', $servi�o_id, PDO::PARAM_INT);
    ...
    foreach ($webhooks as $url) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            'X-Mentalpag-Event: ' . $trigger_event, // Cabeçalho personalizado
            'User-Agent: MentalpagWebhookAgent/1.0' // User-Agent personalizado
        ]);
 ...
 */
function trigger_utmfy_integrations($usuario_id, $event_data, $trigger_event, $servi�o_id = null, $mercado_pago_payment_data = []) {
    global $pdo;
    log_webhook("UTMFY: Verificando integrações para o evento '$trigger_event' (usuario_id: $usuario_id, servi�o_id: " . ($servi�o_id ?? 'NULL') . ")");
    ...
    $stmt_integrations->bindParam(':product_id', $servi�o_id, PDO::PARAM_INT);
    ...
    // Mapeamento de status da Mentalpag para status da UTMfy
    $status_map = [
        'approved'     => 'paid',
    ...
    // Mapeamento de métodos de pagamento da Mentalpag para UTMfy
    $payment_method_map = [
        'Pix' => 'pix',
        'Boleto' => 'boleto',
    ...
    foreach ($integrations as $api_token) {
        ...
        foreach ($event_data['servi�os_comprados'] as $product) {
            $product_price_in_cents = (int)(((float)$product['valor']) * 100);
            $products_payload[] = [
                'id' => (string)$product['servi�o_id'],
                'name' => (string)$product['nome'],
                'planId' => null, // Assumindo que Mentalpag não tem 'planId'
                'planName' => null, // Assumindo que Mentalpag não tem 'planName'
        ...
        if (empty($created_at_utc)) {
            ...
            log_webhook("UTMFY: data_venda da Mentalpag vazia, usando tempo atual UTC para createdAt.");
        }


        $payload = [
            'orderId' => (string)$event_data['transacao_id'],
            'platform' => 'MentalpagBR6', // Nome da sua plataforma
            ...
            'commission' => [
                ...
                'gatewayFeeInCents' => 0, // Ajustar se a Mentalpag tiver uma taxa de gateway fixa/conhecida
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    log_webhook("  Iniciando processamento de entrega para servi�o '$product_name'. Tipo: '$delivery_type'.");
        ...
        case 'area_membros':
            if (!empty($customer_email) && !empty($product_id_for_area_membros)) {
                ...
                if ($stmt_grant_access->rowCount() > 0) {
                    log_webhook("    SUCESSO DE ENTREGA (Área de Membros): Acesso concedido para " . $customer_email . " ao servi�o ID " . $product_id_for_area_membros);
                    return ['success' => true, 'product_name' => $product_name, 'content_type' => 'area_membros', 'content_value' => null];
                } else {
                    log_webhook("    INFO DE ENTREGA (Área de Membros): Acesso para " . $customer_email . " ao servi�o ID " . $product_id_for_area_membros . " já existia ou falhou (IGNORADO).");
...
function send_delivery_email_consolidated($to_email, $customer_name, $processed_products_for_email, $member_area_password, $member_area_login_url, $email_subject, $email_html_template) {
    ...
    try {
        ...
        $smtp_config = [
            ...
            'from_name' => $smtp_configs_raw['smtp_from_name'] ?? 'Mentalpag'
        ];
...
/**
 * Cria uma notificação no banco de dados para o infoservi�or.
 *
 * @param int $usuario_id ID do infoservi�or.
...
function create_notification($usuario_id, $tipo, $mensagem, $valor, $venda_id_fk = null, $metodo_pagamento = null) {
    global $pdo;
    try {
        $link_acao = ($venda_id_fk) ? "index.php?pagina=vendas&id={$venda_id_fk}" : null;
        ...
        error_log("ERRO: Falha ao criar notificação para o infoservi�or ID $usuario_id: " . $e->getMessage());
    }
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($data['type']) && $data['type'] === 'payment' && isset($data['data']['id'])) {
            ...
            try {
                // 1. Encontrar o servi�o_id e o usuario_id (infoservi�or) associados a ESTA transação
                $stmt_get_venda_base_info = $pdo->prepare("
                    SELECT v.id as venda_db_id, v.servi�o_id, p.usuario_id, v.checkout_session_uuid, v.comprador_email, v.comprador_nome, v.comprador_telefone, v.comprador_cpf, p.checkout_hash,
                ...
                if (!$venda_base_info) {
                    log_webhook("Info: Transação ID '$payment_id' não encontrada no BD ou sem servi�o/usuário associado. Ignorando.");
                    exit();
                }

                $usuario_id_do_servi�or = $venda_base_info['usuario_id'];
                ...
                // 2. Obter o access_token do Mercado Pago para o infoservi�or dono do servi�o
                $stmt_get_mp_token = $pdo->prepare("SELECT mp_access_token FROM usuarios WHERE id = ?");
                $stmt_get_mp_token->execute([$usuario_id_do_servi�or]);
                $access_token = $stmt_get_mp_token->fetchColumn();

                if (empty($access_token)) {
                    log_webhook("ERRO CRÍTICO: Access Token do Mercado Pago não configurado para o infoservi�or ID: $usuario_id_do_servi�or. Não é possível consultar a API do MP.");
                    exit();
                }
                ...
                log_webhook("Mercado Pago API: Consultando payment ID '$payment_id' para infoservi�or '$usuario_id_do_servi�or'.");
                ...
                if ($http_code == 200) {
                    ...
                    if ($all_sales_details) {
                        ...
                        $webhook_payload = [
                            'infoservi�or_id' => $usuario_id_do_servi�or,
                        ...
                        foreach ($all_sales_details as $sale_item) {
                            $webhook_payload['servi�os_comprados'][] = [
                                'servi�o_id' => $sale_item['servi�o_id'],
                                'nome' => $sale_item['servi�o_nome'],
                        ...
                        trigger_webhooks($usuario_id_do_servi�or, $webhook_payload, $new_status);
                        trigger_webhooks($usuario_id_do_servi�or, $webhook_payload, $new_status, $main_sale_details['servi�o_id']);
                        foreach ($all_sales_details as $sale_item) {
                            if ($sale_item['servi�o_id'] !== $main_sale_details['servi�o_id']) {
                                trigger_webhooks($usuario_id_do_servi�or, $webhook_payload, $new_status, $sale_item['servi�o_id']);
                            }
                        }

                        // --- NOVO: Lógica para UTMfy ---
                        // Passar os dados completos do Mercado Pago para a UTMfy
                        trigger_utmfy_integrations($usuario_id_do_servi�or, $webhook_payload, $new_status, null, $payment_data); // Global
                        trigger_utmfy_integrations($usuario_id_do_servi�or, $webhook_payload, $new_status, $main_sale_details['servi�o_id'], $payment_data); // Servi�o principal
                        foreach ($all_sales_details as $sale_item) {
                            if ($sale_item['servi�o_id'] !== $main_sale_details['servi�o_id']) {
                                trigger_utmfy_integrations($usuario_id_do_servi�or, $webhook_payload, $new_status, $sale_item['servi�o_id'], $payment_data); // Order bumps
                            }
                        }
                        ...
                        $valor_total_compra = array_sum(array_column($all_sales_details, 'valor'));
                        $servi�os_nomes = implode(', ', array_unique(array_column($all_sales_details, 'servi�o_nome')));
                        $metodo_pagamento_venda = htmlspecialchars($main_sale_details['metodo_pagamento']);

                        switch ($new_status) {
                            case 'approved':
                                $notification_type = 'Compra Aprovada';
                                $notification_message = "Parabéns! Nova venda aprovada de R$" . number_format($valor_total_compra, 2, ',', '.') . " para os servi�os '{$servi�os_nomes}' via {$metodo_pagamento_venda}.";
                                break;
                            case 'pending':
                                if ($main_sale_details['metodo_pagamento'] === 'Pix') {
                                    $notification_type = 'Pix Gerado';
                                    $notification_message = "Um pagamento Pix de R$" . number_format($valor_total_compra, 2, ',', '.') . " foi gerado para os servi�os '{$servi�os_nomes}'. Aguardando pagamento.";
                                } elseif ($main_sale_details['metodo_pagamento'] === 'Boleto') {
                                    $notification_type = 'Boleto Gerado';
                                    $notification_message = "Um boleto de R$" . number_format($valor_total_compra, 2, ',', '.') . " foi gerado para os servi�os '{$servi�os_nomes}'. Aguardando pagamento.";
                                } else {
                                    $notification_type = 'Pagamento Pendente';
                                    $notification_message = "Um pagamento de R$" . number_format($valor_total_compra, 2, ',', '.') . " para os servi�os '{$servi�os_nomes}' está pendente.";
                                }
                                break;
                            case 'in_process':
                                $notification_type = 'Pagamento em Processamento';
                                $notification_message = "O pagamento de R$" . number_format($valor_total_compra, 2, ',', '.') . " para os servi�os '{$servi�os_nomes}' está em processamento.";
                                break;
                            case 'rejected':
                                $notification_type = 'Pagamento Recusado';
                                $notification_message = "O pagamento de R$" . number_format($valor_total_compra, 2, ',', '.') . " para os servi�os '{$servi�os_nomes}' foi recusado. Motivo: " . htmlspecialchars($payment_data['status_detail'] ?? 'Não informado.');
                                break;
                            case 'cancelled':
                                $notification_type = 'Pagamento Cancelado';
                                $notification_message = "O pagamento de R$" . number_format($valor_total_compra, 2, ',', '.') . " para os servi�os '{$servi�os_nomes}' foi cancelado.";
                                break;
                            case 'refunded':
                                $notification_type = 'Reembolso';
                                $notification_message = "Um reembolso de R$" . number_format($valor_total_compra, 2, ',', '.') . " foi emitido para a venda dos servi�os '{$servi�os_nomes}'.";
                                break;
                            case 'charged_back':
                                $notification_type = 'Chargeback';
                                $notification_message = "ATENÇÃO: Foi detectado um chargeback de R$" . number_format($valor_total_compra, 2, ',', '.') . " para a venda dos servi�os '{$servi�os_nomes}'.";
                                break;
                            default:
                                $notification_type = 'Atualização de Pagamento';
                                $notification_message = "Status do pagamento de R$" . number_format($valor_total_compra, 2, ',', '.') . " para '{$servi�os_nomes}' atualizado para: " . ucfirst(str_replace('_', ' ', $new_status)) . ".";
                                break;
                        }
                        
                        create_notification($usuario_id_do_servi�or, $notification_type, $notification_message, $valor_total_compra, $primeira_venda_db_id, $metodo_pagamento_venda);

                        // --- Lógica de Entrega de Servi�os (Somente para Aprovados e se o e-mail ainda não foi enviado) ---
                        if ($new_status === 'approved') {
                            // Tenta marcar como enviado ATOMICAMENTE para evitar race condition (emails duplicados)
                            $stmt_lock = $pdo->prepare("UPDATE vendas SET email_entrega_enviado = 1 WHERE checkout_session_uuid = ? AND email_entrega_enviado = 0");
                            $stmt_lock->execute([$checkout_session_uuid]);

                            if ($stmt_lock->rowCount() > 0) {
                                log_webhook("Entrega: Bloqueio adquirido para envio de e-mail da sessão '$checkout_session_uuid'. Processando entrega.");
                                ...
                                foreach ($all_sales_details as $sale) {
                                    ...
                                    if ($delivery_result['success']) {
                                        if ($delivery_result['content_type'] === 'area_membros') {
                                            if ($member_area_password_for_delivery === null) {
                                                ...
                                                if ($existing_user) {
                                                    // User exists. Do NOT update password.
                                                    log_webhook("  Área de Membros: Usuário já existe. Mantendo senha atual para: " . $customer_email);
                                                    $member_area_password_for_delivery = "Utilize sua senha de acesso já cadastrada.";
                                                } else {
                                                    $new_password = bin2hex(random_bytes(8));
                                                    $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
                                                    
                                                    $stmt_insert_user = $pdo->prepare("INSERT INTO usuarios (usuario, nome, senha, tipo) VALUES (?, ?, ?, 'usuario')");
                                                    $stmt_insert_user->execute([$customer_email, $customer_name, $hashed_password]);
                                                    log_webhook("  Área de Membros: Novo usuário criado: " . $customer_email);
                                                    $member_area_password_for_delivery = $new_password;
                                                }
                                        ...
                                        log_webhook("Entrega falhou para o servi�o '{$sale['servi�o_nome']}': {$delivery_result['message']}");
                                    }
                                }

                                if (!empty($processed_products_for_email)) {
                                    log_webhook("EMAIL_DELIVERY: Tentando enviar e-mail consolidado para '$customer_email' com " . count($processed_products_for_email) . " servi�os.");
                                    log_webhook("EMAIL_DELIVERY: Conteúdo dos servi�os para e-mail: " . print_r($processed_products_for_email, true));
                                    ...
                                    if ($email_sent) {
                                        // Email sent successfully. Status is already 1.
                                        log_webhook("Entrega: E-mail enviado com sucesso para '$customer_email'.");
                                    } else {
                                        // Revert status to 0 if email failed
                                        $pdo->prepare("UPDATE vendas SET email_entrega_enviado = 0 WHERE checkout_session_uuid = ?")->execute([$checkout_session_uuid]);
                                        log_webhook("Entrega: Falha no envio do e-mail consolidado para '$customer_email'. Status revertido para 0.");
                                    }
                                } else {
                                    log_webhook("Entrega: Nenhum conteúdo para e-mail consolidado para a sessão '$checkout_session_uuid'.");
                                }
                            } else {
                                log_webhook("Entrega: E-mail consolidado já enviado para a sessão '$checkout_session_uuid' ou bloqueio falhou. Ignorando reenvio.");
                            }
                        }
                    } else {
                        log_webhook("ERRO DE LÓGICA: Não foi possível encontrar detalhes do servi�o/venda para a transação '$transaction_id' e sessão '$checkout_session_uuid'.");
                    }
        ...
        try {
            ...
            if ($action === 'get_live_notifications') {
                $stmt = $pdo->prepare("
                    SELECT 
                        n.id, n.tipo, n.mensagem, n.valor, n.metodo_pagamento, p.nome as servi�o_nome, p.foto as servi�o_foto 
                    FROM notificacoes n
                    LEFT JOIN vendas v ON n.venda_id_fk = v.id
                    LEFT JOIN servi�os p ON v.servi�o_id = p.id
