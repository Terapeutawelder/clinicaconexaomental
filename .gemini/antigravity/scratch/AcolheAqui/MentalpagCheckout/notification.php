...
if (file_exists('utmfy_helper.php')) require_once 'utmfy_helper.php';
...
function handle_tracking_events($status, $sale_details, $checkout_config) {
    ...
    if (!empty($tracking_config['events']['facebook'][$event_info['key']])) {
        ...
        $checkout_url = $protocol . $_SERVER['HTTP_HOST'] . rtrim(dirname($_SERVER['PHP_SELF']), '/\\') . '/checkout.php?p=' . $sale_details['checkout_hash'];
...
function trigger_webhooks($usuario_id, $event_data, $trigger_event, $servi�o_id = null) {
    ...
    $stmt = $pdo->prepare("SELECT url FROM webhooks WHERE usuario_id = :uid AND {$event_field} = 1 AND (servi�o_id IS NULL OR servi�o_id = :pid)");
    $stmt->execute([':uid' => $usuario_id, ':pid' => $servi�o_id]);
    ...
    foreach ($webhooks as $url) {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json', 'X-Mentalpag-Event: ' . $trigger_event]);
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    if ($type === 'link' && !empty($content)) return ['success' => true, 'product_name' => $product_data['servi�o_nome'], 'content_type' => 'link', 'content_value' => $content];
    if ($type === 'email_pdf' && !empty($content) && file_exists('uploads/' . $content)) return ['success' => true, 'product_name' => $product_data['servi�o_nome'], 'content_type' => 'pdf', 'content_value' => 'uploads/' . $content];
    if ($type === 'area_membros') {
        $pdo->prepare("INSERT IGNORE INTO alunos_acessos (aluno_email, servi�o_id) VALUES (?, ?)")->execute([$customer_email, $product_data['servi�o_id']]);
        return ['success' => true, 'product_name' => $product_data['servi�o_nome'], 'content_type' => 'area_membros', 'content_value' => null];
    }
...
function send_delivery_email_consolidated($to_email, $customer_name, $products, $pass, $login_url) {
    ...
    try {
        ...
        $mail->setFrom($fromEmail, $config['smtp_from_name'] ?? 'Mentalpag');
        ...
        $template = $config['email_template_delivery_html'] ?? '<p>Olá {CLIENT_NAME}, aqui estão seus servi�os:</p><!-- LOOP_PRODUCTS_START --><p>{PRODUCT_NAME}</p><!-- LOOP_PRODUCTS_END -->';
...
function create_notification($usuario_id, $tipo, $mensagem, $valor, $venda_id_fk = null, $metodo = null) {
    ...
    try {
        $link = $venda_id_fk ? "index.php?pagina=vendas&id={$venda_id_fk}" : null;
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($payment_id) {
            $stmt = $pdo->prepare("SELECT v.*, p.usuario_id, u.mp_access_token FROM vendas v JOIN servi�os p ON v.servi�o_id = p.id LEFT JOIN usuarios u ON p.usuario_id = u.id WHERE v.transacao_id = ? LIMIT 1");
            ...
            if ($venda) {
                ...
                if ($new_status) {
                    ...
                    $stmt_all = $pdo->prepare("
                        SELECT v.*, p.usuario_id, p.nome as servi�o_nome, p.tipo_entrega, p.conteudo_entrega, p.checkout_config, p.checkout_hash 
                        FROM vendas v 
                    ...
                    if (!empty($all_sales)) {
                        ...
                        if (function_exists('trigger_utmfy_integrations')) {
                            ...
                            trigger_utmfy_integrations($main_sale['usuario_id'], $webhook_payload, $db_status, $main_sale['servi�o_id']);
                        }
                        ...
                        if ($db_status === 'approved') {
                            ...
                            $stmt_lock->execute([$main_sale['checkout_session_uuid']]);

                            if ($stmt_lock->rowCount() > 0) {
                                log_webhook("EMAIL LOCK: Bloqueio adquirido para envio de e-mail da sessão " . $main_sale['checkout_session_uuid']);
                                ...
                                foreach ($all_sales as $s) {
                                    ...
                                    if ($res['success']) {
                                        if ($res['content_type'] == 'area_membros') {
                                            ...
                                            if ($u = $exists->fetch()) {
                                                // NÃO RESETA A SENHA SE USUÁRIO JÁ EXISTE (Conforme solicitado)
                                                // $pdo->prepare("UPDATE usuarios SET senha = ? WHERE id = ?")->execute([$pass_hash, $u['id']]);
                                                $pass = "Utilize sua senha de acesso já cadastrada."; 
                                            } else {
                                ...
                                if (!empty($processed_prods)) {
                                    ...
                                    $sent_success = send_delivery_email_consolidated($main_sale['comprador_email'], $main_sale['comprador_nome'], $processed_prods, $pass, $login_url);
                                    
                                    if (!$sent_success) {
                                        // Se falhar o envio, reverte o status para 0 para tentar novamente ou permitir retry manual
                                        $pdo->prepare("UPDATE vendas SET email_entrega_enviado = 0 WHERE checkout_session_uuid = ?")->execute([$main_sale['checkout_session_uuid']]);
                                        log_webhook("EMAIL ERROR: Falha ao enviar e-mail. Status revertido para 0.");
                                    } else {
                                        log_webhook("EMAIL SUCCESS: E-mail enviado e registrado.");
                                    }
                                }
                            } else {
                                log_webhook("EMAIL SKIP: E-mail já enviado ou sendo processado por outra requisição (Lock failed).");
                            }
                        }
                    }
                }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($action === 'get_live_notifications') {
            $s = $pdo->prepare("SELECT n.id, n.tipo, n.mensagem, n.valor, n.metodo_pagamento, p.nome as servi�o_nome, p.foto as servi�o_foto FROM notificacoes n LEFT JOIN vendas v ON n.venda_id_fk = v.id LEFT JOIN servi�os p ON v.servi�o_id = p.id WHERE n.usuario_id = ? AND n.displayed_live = 0 ORDER BY n.data_notificacao ASC LIMIT 5");
            $s->execute([$uid]);
