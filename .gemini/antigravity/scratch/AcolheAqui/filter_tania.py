...
output_file = "Atendimentos_Tania_Dezembro_2025.csv"
encoding = "latin-1" 
...
try:
    with open(input_file, 'r', encoding=encoding) as f_in, \
        ...
        for row in reader:
             ...
             if 'Dra.Tania' in owner_val:
                 if is_dec_2025(date_val):
                     writer.writerow(row)
                     count += 1
                 
    print(f"Successfully saved {count} appointments for 'Dra.Tania' in December 2025 to '{output_file}'")
