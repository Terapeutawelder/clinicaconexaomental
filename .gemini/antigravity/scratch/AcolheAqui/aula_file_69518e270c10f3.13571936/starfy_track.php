...
try {
    ...
    // Busca produtos já rastreados pelo infoprodutor
    ...
    $mensagem = "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>Erro ao buscar produtos: " . htmlspecialchars($e->getMessage()) . "</div>";
...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Starfy Track</h1>
    <p class="text-gray-600 mb-8">Monitore o desempenho do seu funil de vendas em tempo real. Veja quantas pessoas visitam sua página, chegam ao checkout e compram seus produtos.</p>

    <?php echo $mensagem; ?>

    <!-- Configuração de Rastreamento -->
    <div class="bg-white p-8 rounded-lg shadow-md mb-8">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Configurar Rastreamento de Produto</h2>
        <div class="space-y-6">
            <div>
                <label for="product_select" class="block text-gray-700 text-sm font-semibold mb-2">Selecione um Produto para Rastrear</label>
                <div class="flex flex-col sm:flex-row items-stretch sm:space-x-4 space-y-4 sm:space-y-0">
                    <select id="product_select" class="w-full sm:w-2/3 px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                    ...
                    <button id="add_track_product_btn" class="w-full sm:w-1/3 bg-orange-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-orange-700 transition duration-300 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
                        <i data-lucide="plus-circle" class="w-5 h-5"></i>
                        <span>Ativar Rastreamento</span>
                    </button>
                </div>
                <div id="product_select_error" class="text-red-500 text-sm mt-2 hidden">Por favor, selecione um produto.</div>
            </div>

            <!-- Produtos já rastreados -->
            <div class="mt-8 border-t pt-6">
                <h3 class="text-xl font-semibold text-gray-800 mb-4">Produtos Ativamente Rastreando</h3>
    ...
    <div id="tracking_data_section" class="bg-white p-8 rounded-lg shadow-md" style="display: none;">
        <div class="flex items-center justify-between mb-6 border-b pb-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-800">Análise de Funil para <span id="analyzed_product_name" class="text-orange-600"></span></h2>
        ...
        <div class="flex flex-wrap items-center justify-start gap-3 mb-6 bg-gray-100 p-2 rounded-lg">
            ...
            <button data-period="today" class="period-filter-btn px-3 py-1 text-sm font-semibold rounded-md bg-orange-500 text-white shadow">Hoje</button>
            ...
            <div class="lg:w-1/2 w-full space-y-4">
                ...
                <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded-lg shadow-sm">
                 ...
                 <div class="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg shadow-sm">
                    ...
                    <p id="conversion_overall" class="text-3xl font-bold text-orange-800">0%</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                <h3 class="text-gray-500 text-sm font-medium">Vendas do Produto Principal</h3>
...
<div id="script_modal" class="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl transform transition-all opacity-0 scale-95" id="script_modal_content">
        <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-2xl font-bold text-gray-800">Script de Rastreamento para <span id="script_product_name" class="text-orange-600"></span></h2>
        ...
        <div class="p-6 space-y-4">
            ...
            <p class="text-sm text-gray-500">Este script irá rastrear visitas à página e cliques em botões de checkout para o produto selecionado.</p>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addTrackProductBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('api.php?action=add_starfy_tracked_product', {
                method: 'POST',
    ...
    async function refreshTrackedProductsList() {
        try {
            const response = await fetch('api.php?action=get_starfy_tracked_products');
            const result = await response.json();
    ...
    periodFilterButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            this.classList.add('bg-orange-500', 'text-white', 'shadow');
    ...
    async function fetchTrackingData(trackingProductDbId, period) {
        try {
            const response = await fetch(`api.php?action=get_starfy_tracking_data&tracking_product_id=${trackingProductDbId}&period=${period}`);
            const result = await response.json();
    ...
    trackedProductsList.addEventListener('click', async function(e) {
        ...
        if (deleteBtn) {
            ...
            if (confirm(`Tem certeza que deseja excluir o funil de rastreamento para o produto "${productName}"? Esta ação não pode ser desfeita.`)) {
                try {
                    const response = await fetch('api.php?action=delete_starfy_tracked_product', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
