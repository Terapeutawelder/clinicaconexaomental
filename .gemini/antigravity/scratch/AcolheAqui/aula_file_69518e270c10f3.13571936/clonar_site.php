...
<div class="container mx-auto p-4 lg:p-8 max-w-7xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Clonador de Sites</h1>
            <p class="text-gray-500 mt-1">Copie, edite e lance páginas de vendas em segundos.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-orange-100 overflow-hidden mb-8 transition-all hover:shadow-xl">
            ...
            <form id="cloneForm" class="relative">
                <div class="flex flex-col md:flex-row gap-4 items-end">
                    <div class="flex-grow w-full">
                        <label for="url_to_clone" class="block text-sm font-semibold text-gray-700 mb-1">URL do Site Original</label>
                        <div class="relative rounded-md shadow-sm">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="text-gray-400 sm:text-sm">https://</span>
                            </div>
                            <input type="url" id="url_to_clone" name="url_to_clone" class="focus:ring-orange-500 focus:border-orange-500 block w-full pl-16 sm:text-sm border-gray-300 rounded-lg py-3" placeholder="www.exemplo.com/pagina" required>
                        </div>
                    </div>
                    <button type="submit" class="w-full md:w-auto flex items-center justify-center py-3 px-6 border border-transparent text-sm font-medium rounded-lg text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors shadow-md hover:shadow-lg shadow-orange-200">
    ...
    <div id="editorSection" class="hidden bg-white rounded-xl shadow-lg border border-orange-100 overflow-hidden mb-8">
        <div class="border-b border-gray-100 bg-orange-50/30 p-6">
            <div class="flex flex-col lg:flex-row justify-between lg:items-center gap-6">
                
                <!-- Título e Slug -->
                <div class="flex-grow space-y-4 w-full lg:w-2/3">
                    <div>
                        <label for="clonedSiteTitle" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Nome do Projeto</label>
                        <input type="text" id="clonedSiteTitle" class="block w-full border-gray-200 bg-white rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-lg font-medium p-2" placeholder="Ex: Página de Vendas Black Friday">
                    </div>

                    <!-- Slug Pequenininho -->
                    <div class="flex items-center">
                        <label for="clonedSiteSlug" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mr-3 whitespace-nowrap">Link:</label>
                        <div class="flex rounded-md shadow-sm w-full max-w-sm">
                            <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-100 text-gray-500 text-xs">
                                /s/
                            </span>
                            <input type="text" id="clonedSiteSlug" class="focus:ring-orange-500 focus:border-orange-500 flex-1 block w-full rounded-none rounded-r-md text-xs border-gray-300 py-1.5" placeholder="minha-pagina">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-end gap-3 min-w-[220px]">
                    <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Publicação</span>
                    
                    <!-- Container dos Botões -->
                    <div class="flex flex-col gap-2 w-full">
                        <!-- Botão Publicar (Destaque quando rascunho) -->
                        <button type="button" id="btnStatusPublished" class="group relative w-full flex items-center justify-center px-4 py-2 border border-transparent text-sm font-bold rounded-lg text-white bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 shadow-md transition-all transform hover:-translate-y-0.5">
                        ...
                        <button type="button" id="btnStatusDraft" class="hidden w-full flex items-center justify-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-all">
                    ...
                    <div id="statusIndicator" class="flex items-center text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                        <i data-lucide="edit-3" class="w-5 h-5 mr-2 text-orange-500"></i> Editor Visual
                    </h3>
                    <p class="text-xs text-gray-500 mt-1">Clique nos elementos abaixo para editar texto ou links.</p>
                </div>
            </div>
            
            <div id="editorStatus" class="text-sm text-gray-500 italic mb-2"></div>

            <!-- Barra de ferramentas do editor visual -->
            <div id="editorToolbar" class="flex flex-wrap gap-2 p-3 bg-gray-50 border border-gray-200 rounded-t-xl sticky top-0 z-10">
                <div class="flex gap-1 border-r border-gray-300 pr-2 mr-1">
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="bold" title="Negrito"><i data-lucide="bold" class="w-4 h-4"></i></button>
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="italic" title="Itálico"><i data-lucide="italic" class="w-4 h-4"></i></button>
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="underline" title="Sublinhado"><i data-lucide="underline" class="w-4 h-4"></i></button>
                </div>
                
                <div class="flex gap-1 border-r border-gray-300 pr-2 mr-1">
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="createLink" title="Inserir Link"><i data-lucide="link" class="w-4 h-4"></i></button>
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="unlink" title="Remover Link"><i data-lucide="link-2-off" class="w-4 h-4"></i></button>
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-700 transition-all" data-command="insertImage" title="Imagem"><i data-lucide="image" class="w-4 h-4"></i></button>
                </div>

                <div class="flex gap-1 items-center border-r border-gray-300 pr-2 mr-1">
                    <label class="flex items-center justify-center p-2 hover:bg-white rounded hover:shadow-sm cursor-pointer text-gray-700 transition-all" title="Cor do Texto">
                        <div class="w-4 h-4 rounded-full bg-gradient-to-br from-red-500 to-orange-500 border border-gray-300 mr-1"></div>
                        <input type="color" id="foreColorPicker" class="opacity-0 absolute w-0 h-0" value="#000000">
                        <i data-lucide="chevron-down" class="w-3 h-3 text-gray-400"></i>
                    </label>
                </div>

                <select class="px-3 py-1.5 bg-white border border-gray-300 rounded text-gray-700 text-sm focus:outline-none focus:ring-1 focus:ring-orange-500" data-command="formatBlock">
                ...
                <div class="flex gap-1">
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-600" data-command="undo"><i data-lucide="undo" class="w-4 h-4"></i></button>
                    <button type="button" class="p-2 hover:bg-white rounded hover:shadow-sm text-gray-600" data-command="redo"><i data-lucide="redo" class="w-4 h-4"></i></button>
                    <button type="button" id="deleteElementBtn" class="p-2 bg-red-50 hover:bg-red-100 rounded text-red-600 border border-red-200 ml-2" title="Deletar Seleção"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                </div>
            </div>

            <!-- Iframe para edição visual -->
            <div class="relative group border border-gray-300 rounded-b-xl overflow-hidden bg-white">
                <iframe id="visualEditorFrame" src="about:blank" class="w-full h-[650px] bg-white"></iframe>
                
                <!-- Loading Overlay -->
                <div id="editorLoadingOverlay" class="absolute inset-0 bg-white/80 z-20 flex items-center justify-center hidden">
                    <div class="text-center">
                        <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-orange-500 mx-auto mb-2"></div>
                        <p class="text-sm text-gray-600">Preparando editor...</p>
                    </div>
                </div>
            </div>

            <!-- Footer Actions -->
            <div class="mt-6 flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-gray-100">
                 ...
                 <div class="flex gap-3 order-1 md:order-2 w-full md:w-auto">
                    <button id="copyHtmlFromEditorBtn" class="flex-1 md:flex-none relative py-2.5 px-5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all overflow-hidden">
                        <span class="button-content flex items-center justify-center">
                            <i data-lucide="code" class="w-4 h-4 mr-2 text-gray-500"></i> Copiar Código
                        </span>
                        <span class="feedback-overlay absolute inset-0 flex items-center justify-center bg-gray-800 text-white opacity-0 transition-opacity duration-300 pointer-events-none"></span>
                    </button>

                    <button id="saveVisualEditorBtn" class="flex-1 md:flex-none relative py-2.5 px-8 border border-transparent shadow-lg shadow-orange-200 text-sm font-bold rounded-lg text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all overflow-hidden">
                        <span class="button-content flex items-center justify-center">
        ...
        <div class="border-t border-gray-100 bg-gray-50 p-6">
            <details class="group">
                <summary class="flex justify-between items-center font-medium cursor-pointer list-none text-gray-700 hover:text-orange-600 transition-colors">
                    <span class="flex items-center">
                        <i data-lucide="settings" class="w-5 h-5 mr-2 text-gray-400 group-hover:text-orange-500"></i>
                ...
                <div class="text-gray-500 mt-4 animate-fadeIn">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                        <div>
                            <label for="facebookPixelId" class="block text-xs font-semibold text-gray-600 uppercase mb-1">Facebook Pixel ID</label>
                            <input type="text" id="facebookPixelId" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm p-2" placeholder="Ex: 1234567890">
                        </div>
                        <div>
                            <label for="googleAnalyticsId" class="block text-xs font-semibold text-gray-600 uppercase mb-1">Google Analytics (G- / UA-)</label>
                            <input type="text" id="googleAnalyticsId" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm p-2" placeholder="Ex: G-XXXXXXXXXX">
                        </div>
                    </div>
                    
                    <div>
                        <label for="customHeadScripts" class="block text-xs font-semibold text-gray-600 uppercase mb-1">Scripts Personalizados (Head)</label>
                        <textarea id="customHeadScripts" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm p-2 font-mono h-24" placeholder="Cole aqui seus scripts do Taboola, Outbrain, Chat, etc..."></textarea>
                    </div>
                </div>
            </details>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-orange-100 overflow-hidden">
        <div class="p-6 md:p-8 border-b border-gray-100">
            <h2 class="text-xl font-bold text-gray-800 flex items-center">
                <i data-lucide="layers" class="w-5 h-5 mr-3 text-orange-500"></i> 
                Minha Biblioteca
            </h2>
        </div>
        <div id="clonedSitesList" class="divide-y divide-gray-100">
...
<div id="floatingEditorBox" class="fixed hidden flex-col p-4 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 w-80 backdrop-blur-sm bg-white/95 transition-opacity duration-200" style="top:0; left:0;">
    <div class="flex justify-between items-center mb-3 border-b border-gray-100 pb-2">
        <span class="text-xs font-bold text-gray-500 uppercase flex items-center">
            <i data-lucide="edit-2" class="w-3 h-3 mr-1 text-orange-500"></i> Editar Elemento
        </span>
        <button id="closeFloatingEditor" class="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded hover:bg-gray-100"><i data-lucide="x" class="w-4 h-4"></i></button>
    </div>
    
    <div class="space-y-3">
        <div id="textEditGroup">
            <label class="block text-xs font-medium text-gray-700 mb-1">Conteúdo de Texto</label>
            <textarea id="floatingEditorText" class="block w-full border-gray-300 rounded-lg shadow-sm text-sm focus:ring-orange-500 focus:border-orange-500 resize-none h-24 p-2 custom-scrollbar"></textarea>
        </div>

        <div id="urlEditGroup">
            <label class="block text-xs font-medium text-gray-700 mb-1">Link de Destino (URL)</label>
            <input type="text" id="floatingEditorUrl" class="block w-full border-gray-300 rounded-lg shadow-sm text-sm focus:ring-orange-500 focus:border-orange-500 p-2" placeholder="https://...">
        </div>
        
        <div id="imageEditGroup" class="hidden">
            <label class="block text-xs font-medium text-gray-700 mb-1">URL da Imagem</label>
            <input type="text" id="floatingEditorImgSrc" class="block w-full border-gray-300 rounded-lg shadow-sm text-sm focus:ring-orange-500 focus:border-orange-500 p-2" placeholder="https://...">
        </div>
    </div>

    <div class="flex justify-end gap-2 mt-4 pt-3 border-t border-gray-100">
        <button id="deleteFloatingElementBtn" class="p-2 text-red-500 hover:bg-red-50 rounded-md transition-colors" title="Excluir Elemento">
            <i data-lucide="trash-2" class="w-4 h-4"></i>
        </button>
        <button id="saveFloatingEditorBtn" class="flex-1 inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 shadow-sm transition-all">
            Salvar Alteração
        </button>
    </div>
</div>
...
<div id="publishModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity opacity-0 pointer-events-none">
    <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full transform scale-95 transition-transform relative" id="publishModalContent">
        
        <!-- Botão Fechar (X) -->
        <button id="closePublishModalX" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors focus:outline-none">
        ...
        <div id="publishStateLoading" class="text-center mt-4">
            <div class="w-20 h-20 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse">
                <i data-lucide="rocket" class="w-10 h-10 text-orange-600"></i>
            </div>
            <h3 class="text-2xl font-bold text-gray-800 mb-2">Lançando seu Site</h3>
            <p id="publishStepText" class="text-gray-500 mb-6 h-6">Iniciando compilação...</p>
            
            <!-- Barra de Progresso -->
            <div class="w-full bg-gray-200 rounded-full h-3 mb-2 overflow-hidden">
                <div id="publishProgressBar" class="bg-gradient-to-r from-orange-400 to-red-500 h-3 rounded-full w-0 transition-all duration-300"></div>
            </div>
        ...
        <div id="publishStateSuccess" class="hidden text-center mt-4">
            ...
            <p class="text-gray-500 mb-6">Sua página está publicada e online.</p>
            <button id="viewPublishedSiteBtn" class="w-full py-3 bg-green-600 text-white rounded-lg font-bold shadow-lg hover:bg-green-700 transition-all flex items-center justify-center">
...
<script>
    ...
    function updateStatusUI(status, skipAnimation = false) {
        ...
        if (status === 'published') {
            ...
            // Atualiza o indicador
            statusIndicator.innerHTML = '<span class="w-2 h-2 rounded-full bg-gray-400 mr-2"></span> Status: Rascunho';
            statusIndicator.classList.remove('bg-green-50', 'text-green-700', 'border', 'border-green-200');
            statusIndicator.classList.add('bg-gray-100', 'text-gray-500');
        }
    }
    ...
    function createConfetti() {
        for (let i = 0; i < 50; i++) {
            ...
            confetti.style.backgroundColor = ['#f97316', '#ef4444', '#fbbf24', '#22c55e'][Math.floor(Math.random() * 4)];
    ...
    function showMessage(element, message, type = 'info') {
        ...
        if (type === 'success') {
            element.className = `${baseClasses} bg-green-50 text-green-700 border border-green-200`;
            element.innerHTML = `<i data-lucide="check" class="w-4 h-4 mr-2"></i> ${message}`;
        } else if (type === 'error') {
            element.className = `${baseClasses} bg-red-50 text-red-700 border border-red-200`;
            element.innerHTML = `<i data-lucide="alert-triangle" class="w-4 h-4 mr-2"></i> ${message}`;
        } else {
            element.className = `${baseClasses} bg-blue-50 text-blue-700 border border-blue-200 animate-pulse`;
    ...
    function initializeVisualEditor(htmlContent) {
        ...
        setTimeout(() => {
            ...
            const editorStyles = `
                ::selection { background: #fed7aa; color: #c2410c; }
                img, video, audio, iframe { cursor: pointer; transition: transform 0.2s ease; }
                img:hover, video:hover, audio:hover, iframe:hover, a:hover { outline: 2px dashed #f97316; outline-offset: 2px; }
            ...
            if (editorDoc.body) {
                ...
                editorDoc.body.addEventListener('input', () => {
                    ...
                    editorStatus.classList.add('text-orange-500', 'animate-pulse');
                });
            ...
            editorStatus.classList.remove('text-orange-500', 'animate-pulse');
    ...
    function handleElementClickInEditor(e) {
        ...
        // Efeito de seleção Laranja
        target.style.outline = '3px solid #f97316'; 
        target.style.boxShadow = '0 0 0 4px rgba(249, 115, 22, 0.2)';
    ...
    editorToolbar.addEventListener('click', (e) => {
        ...
        if (button && button.id !== 'deleteElementBtn') {
            ...
            setTimeout(() => button.classList.remove('bg-orange-50', 'text-orange-600'), 200);
        } else if (select) {
    ...
    cloneForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const url = urlToCloneInput.value;
        if (!url) {
        ...
        try {
            ...
            console.error(error);
            showMessage(cloneMessage, 'Falha na conexão.', 'error');
        }
    });
    ...
    saveVisualEditorBtn.addEventListener('click', async () => {
        ...
        allElements.forEach(el => {
            ...
            if (el.style.length > 0) {
                ...
                if (el.style.outline.includes('orange') || el.style.outline.includes('rgb(249, 115, 22)') || el.style.boxShadow.includes('249, 115, 22')) {
                    el.style.outline = '';
                    el.style.boxShadow = '';
    ...
    copyHtmlFromEditorBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`cloned_site_viewer.php?id=${currentClonedSiteId}`);
        ...
        try {
            ...
            if (data.success && data.cloned_sites.length > 0) {
                data.cloned_sites.forEach(site => {
                    ...
                    const statusClass = isPublished ? 'bg-green-100 text-green-800 border-green-200' : 'bg-gray-100 text-gray-800 border-gray-200';
                    const statusText = isPublished ? 'No Ar' : 'Rascunho';
                    
                    const publicLink = (isPublished && site.slug) 
                        ? `<a href="cloned_site_viewer.php?slug=${site.slug}" target="_blank" class="flex items-center mt-2 text-sm text-orange-600 hover:text-orange-800 font-medium transition-colors">
                             <i data-lucide="external-link" class="w-4 h-4 mr-1"></i> Acessar Página
                           </a>` 
                        : '';

                    const card = document.createElement('div');
                    card.className = 'p-6 hover:bg-gray-50 transition-colors flex flex-col md:flex-row justify-between items-start md:items-center group';
                    
                    card.innerHTML = `
                        <div class="flex-1 min-w-0 pr-4">
                            <div class="flex items-center gap-3 mb-1">
                                <h3 class="text-lg font-bold text-gray-900 truncate">${site.title || 'Sem Título'}</h3>
                        ...
                        <div class="flex items-center gap-2 mt-4 md:mt-0 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onclick="editClonedSite(${site.id})" class="p-2 text-gray-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors" title="Editar">
                                <i data-lucide="edit-3" class="w-5 h-5"></i>
                            </button>
                            <button onclick="deleteClonedSite(${site.id})" class="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Excluir">
    ...
    async function deleteClonedSite(id) {
        ...
        try {
            const response = await fetch('api.php?action=delete_cloned_site', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ cloned_site_id: id })
            });
            const data = await response.json();
