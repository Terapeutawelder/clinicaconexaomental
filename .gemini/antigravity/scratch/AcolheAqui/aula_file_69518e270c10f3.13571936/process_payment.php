...
function save_sales($pdo, $data, $main_id, $payment_id, $status, $metodo, $uuid, $utm_params) {
    ...
    try {
        ...
        // Registra o evento de compra diretamente no servidor para garantir que apareça no dashboard
        if ($status === 'approved' || $status === 'pending') { // Registra pending também, ou só approved? Dashboard filtra por approved vendas, mas evento purchase geralmente é approved.
             // Vamos registrar apenas se for 'approved' para consistência com 'purchases' count, 
             // MAS se for Pix/Boleto, o status inicial é pending.
             // O dashboard filtra vendas por status_pagamento='approved'.
             // O evento 'purchase' serve para LIGAR a sessão.
             // Se registrarmos agora, e a venda for aprovada depois (webhook), o link já existe.
             // Então é seguro registrar.
             record_starfy_purchase_server_side($pdo, $main_id, $uuid, $payment_id, $data['transaction_amount']);
        }
        // -----------------------------------------------------------------
...
function record_starfy_purchase_server_side($pdo, $main_product_id, $session_id, $transaction_id, $value) {
    try {
        // Busca o ID de rastreamento interno para este produto
        $stmt = $pdo->prepare("SELECT id FROM starfy_tracking_products WHERE produto_id = ?");
        ...
        if ($tracking_product_id) {
            ...
            // Verifica se já existe para evitar duplicatas (embora o UUID deva ser único)
            $stmt_check = $pdo->prepare("SELECT COUNT(*) FROM starfy_tracking_events WHERE session_id = ? AND event_type = ? AND tracking_product_id = ?");
            $stmt_check->execute([$session_id, $event_type, $tracking_product_id]);
            
            if ($stmt_check->fetchColumn() == 0) {
                $stmt_insert = $pdo->prepare("INSERT INTO starfy_tracking_events (tracking_product_id, session_id, event_type, event_data) VALUES (?, ?, ?, ?)");
