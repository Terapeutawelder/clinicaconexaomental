...
<div class="container mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 mt-1">Visão geral completa da sua plataforma.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div class="bg-white p-5 rounded-xl shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <p id="kpi-total-usuarios" class="text-2xl font-bold text-gray-800">...</p>
            </div>
        </div>
        <div class="bg-white p-5 rounded-xl shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <p id="kpi-faturamento-total" class="text-2xl font-bold text-gray-800">...</p>
            </div>
        </div>
        <div class="bg-white p-5 rounded-xl shadow-sm flex items-center space-x-4">
            ...
            <div>
                <p class="text-gray-500 text-sm">Vendas Aprovadas</p>
                <p id="kpi-vendas-aprovadas" class="text-2xl font-bold text-gray-800">...</p>
            </div>
        </div>
         <div class="bg-white p-5 rounded-xl shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <p id="kpi-produtos-ativos" class="text-2xl font-bold text-gray-800">...</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm mb-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">Picos de Vendas (Últimos 30 dias)</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Produtos Mais Vendidos</h2>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Ranking de Vendedores</h2>
            <div id="top-sellers-list" class="space-y-4">
                <p class="text-gray-500 text-center py-4">Carregando ranking...</p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm">
            <h2 class="text-xl font-semibold text-gray-800 mb-4">Últimos Usuários Cadastrados</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createAvatar(name) {
        ...
        return `
            <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-bold text-lg flex-shrink-0">
                ${initial}
            </div>
        `;
    }
    ...
    function fetchAdminData() {
        fetch('admin_api.php?action=get_admin_dashboard_data')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                    ...
                    data.top_products.forEach(product => {
                        const productHtml = `
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="flex-grow">
                                    <p class="font-semibold text-gray-800 truncate" title="${product.nome}">${product.nome}</p>
                                    <p class="text-sm text-gray-500">${formatCurrency(product.faturamento_total)}</p>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    <p class="font-bold text-lg text-orange-600">${product.total_vendas}</p>
                ...
                if (data.top_sellers && data.top_sellers.length > 0) {
                    data.top_sellers.forEach(seller => {
                        ...
                        const sellerHtml = `
                            <div class="flex items-center space-x-3">
                                ${sellerAvatar}
                                <div class="flex-grow">
                                    <p class="font-semibold text-gray-800 truncate" title="${sellerName}">${sellerName}</p>
                                    <p class="text-sm text-gray-500">${formatCurrency(seller.faturamento_total)}</p>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    <p class="font-bold text-lg text-green-600">${seller.total_vendas}</p>
                                    <p class="text-xs text-gray-400">vendas</p>
                                </div>
                            </div>
                        `;
                    ...
                    topSellersContainer.innerHTML = '<p class="text-gray-500 text-center py-4">Nenhum vendedor com vendas aprovadas.</p>';
                }
                    ...
                    data.recent_users.forEach(user => {
                        ...
                        const userHtml = `
                            <div class="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                                <div class="flex items-center space-x-3">
                                    ${userAvatar}
                                    <p class="font-medium text-gray-700 truncate" title="${userName}">${userName}</p>
                                </div>
                                <span class="text-xs font-semibold px-2 py-1 rounded-full ${user.tipo === 'admin' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}">
                                    ${user.tipo}
                                </span>
                            </div>
                        `;
                     ...
                     recentUsersContainer.innerHTML = '<p class="text-gray-500 text-center py-4">Nenhum usuário encontrado.</p>';
                }
            })
            .catch(error => {
